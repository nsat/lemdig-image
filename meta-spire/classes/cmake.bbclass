...
python() {
    ...
    if "Unix Makefiles" in generator:
        args = "-G '" + generator +  "' -DCMAKE_MAKE_PROGRAM=" + d.getVar("MAKE", True)
...
def oecmake_map_compiler(compiler, d):
    args = d.getVar(compiler, True).split()
    if args[0] == "ccache":
...
set( CMAKE_SYSTEM_NAME ${@map_host_os_to_system_name(d.getVar('HOST_OS', True))} )
set( CMAKE_SYSTEM_PROCESSOR ${@map_host_arch_to_uname_arch(d.getVar('HOST_ARCH', True))} )
...
do_configure[cleandirs] = "${@d.getVar('B', True) if d.getVar('S', True) != d.getVar('B', True) else ''}"
